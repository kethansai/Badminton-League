...
test('login checks credentials and creates an HTTP-only SameSite session without exposing hashes', async () => {
  ...
  assert.deepEqual(Object.keys(result.body).sort(), ['accessToken', 'csrfToken', 'username'])
  await agent.get('/api/admin/session').expect(200)
  await request(app).get('/api/admin/session').set('Authorization', `Bearer ${result.body.accessToken}`).expect(200)
